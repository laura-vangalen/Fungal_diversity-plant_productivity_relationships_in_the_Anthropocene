...
set.seed(435)
sample=spatSample(stack,size=10000,na.rm=T,xy=T) # 10,000 random grid cells
saveRDS(sample,"raster_sample.rds")
...
sample=readRDS("./raster_sample.rds")
sample_new=terra::extract(r1,sample[c("x","y")])
sample_all=cbind(sample,sample_new[c(2:ncol(sample_new))])
saveRDS(sample_all,"./raster_sample.rds")
...
#################################################################################################################################################################
sample=readRDS("raster_sample.rds")
cor(sample[c("ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation","ConsensusLandCoverClass_Urban_Builtup","CSP_Global_Human_Modification","GRIP4_DistanceToAllRoads","GHS_Population_Density")])
# correlations are fine - highest is 0.74 (ConsensusLandCoverClass_Urban_Builtup and CSP global human modification)

sample$EarthEnvTopoMed_Elevation=ifelse(sample$EarthEnvTopoMed_Elevation<0,0,sample$EarthEnvTopoMed_Elevation) # make negative elevations zero
sample$FML_v3.2=as.factor(sample$FML_v3.2) # change to factor
sample=sample[sample$GHS_Population_Density <1500,] # drop population outlier
...
mod1=lm(RES_LOG_arbuscular_mycorrhizal_Richness~CSP_Global_Human_Modification+
          SG_Soil_pH_H2O_005cm+SG_SOC_Content_005cm+CHELSA_BIO_Annual_Mean_Temperature+CHELSA_BIO_Annual_Precipitation+EarthEnvTopoMed_Elevation,data=sample)
summary(mod1)
par(mfrow=c(2,2))
plot(mod1) 
...
# AM
pred_AM=predict(mod1, newdata = data.frame(CSP_Global_Human_Modification = sample$CSP_Global_Human_Modification,
...
# ECM
pred_ECM=predict(mod2, newdata = data.frame(CSP_Global_Human_Modification = sample$CSP_Global_Human_Modification,
                                        SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                        CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant
# Pathogens
pred_path=predict(mod3, newdata = data.frame(CSP_Global_Human_Modification = sample$CSP_Global_Human_Modification,
                                        SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                        CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant

# make df of residuals added to the fit for plotting the partial residuals
partial_effects=data.frame(human_orig=sample$CSP_Global_Human_Modification,pred_AM=pred_AM$fit+resid(mod1),pred_ECM=pred_ECM$fit+resid(mod2),
...
######### for cultivated #########
reference_level <- levels(sample$FML_v3.2)[1] # work out the reference level of the categorical variable

# AM
pred_AM=predict(mod1, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = sample$ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation,
                                           FML_v3.2=reference_level,ConsensusLandCoverClass_Urban_Builtup=0,GHS_Population_Density=0,GRIP4_DistanceToAllRoads=0,
                                           SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                           CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant
# ECM
pred_ECM=predict(mod2, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = sample$ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation,
                                            FML_v3.2=reference_level,ConsensusLandCoverClass_Urban_Builtup=0,GHS_Population_Density=0,GRIP4_DistanceToAllRoads=0,
                                            SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                            CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant
# Pathogens
pred_path=predict(mod3, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = sample$ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation,
                                             FML_v3.2=reference_level,ConsensusLandCoverClass_Urban_Builtup=0,GHS_Population_Density=0,GRIP4_DistanceToAllRoads=0,
                                             SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                             CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant

# make df of residuals added to the fit for plotting the partial residuals
partial_effects=data.frame(human_orig=sample$ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation,pred_AM=pred_AM$fit+resid(mod1),pred_ECM=pred_ECM$fit+resid(mod2),
                           pred_path=pred_path$fit+resid(mod3))
# ggplot format
parital_effects_long=data.frame(pivot_longer(partial_effects,cols=c(2:ncol(partial_effects)),names_to="fungi_type",values_to="residuals"))
parital_effects_long$fungi_type = factor(parital_effects_long$fungi_type, levels=c('pred_AM','pred_ECM','pred_path')) # set order
...
pred_AM=predict(mod1, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = 0,
                                           FML_v3.2=sample$FML_v3.2,ConsensusLandCoverClass_Urban_Builtup=0,GHS_Population_Density=0,GRIP4_DistanceToAllRoads=0,
                                           SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                           CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant
# ECM
...
# make df of residuals added to the fit for plotting the partial residuals
partial_effects=data.frame(human_orig=sample$FML_v3.2,pred_AM=pred_AM$fit+resid(mod1),pred_ECM=pred_ECM$fit+resid(mod2),
                           pred_path=pred_path$fit+resid(mod3))
# ggplot format
...
# AM
pred_AM=predict(mod1, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = 0,
                                           FML_v3.2=reference_level,ConsensusLandCoverClass_Urban_Builtup=sample$ConsensusLandCoverClass_Urban_Builtup,GHS_Population_Density=0,GRIP4_DistanceToAllRoads=0,
                                           SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                           CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant
# ECM
pred_ECM=predict(mod2, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = 0,
...
# make df of residuals added to the fit for plotting the partial residuals
partial_effects=data.frame(human_orig=sample$ConsensusLandCoverClass_Urban_Builtup,pred_AM=pred_AM$fit+resid(mod1),pred_ECM=pred_ECM$fit+resid(mod2),
                           pred_path=pred_path$fit+resid(mod3))
# ggplot format
...
# AM
pred_AM=predict(mod1, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = 0,
                                           FML_v3.2=reference_level,ConsensusLandCoverClass_Urban_Builtup=0,GHS_Population_Density=sample$GHS_Population_Density,GRIP4_DistanceToAllRoads=0,
                                           SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                           CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant
# ECM
pred_ECM=predict(mod2, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = 0,
...
# make df of residuals added to the fit for plotting the partial residuals
partial_effects=data.frame(human_orig=sample$GHS_Population_Density,pred_AM=pred_AM$fit+resid(mod1),pred_ECM=pred_ECM$fit+resid(mod2),
                           pred_path=pred_path$fit+resid(mod3))
# ggplot format
...
# AM
pred_AM=predict(mod1, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = 0,
                                           FML_v3.2=reference_level,ConsensusLandCoverClass_Urban_Builtup=0,GHS_Population_Density=0,GRIP4_DistanceToAllRoads=sample$GRIP4_DistanceToAllRoads,
                                           SG_Soil_pH_H2O_005cm = 0, SG_SOC_Content_005cm = 0, CHELSA_BIO_Annual_Mean_Temperature = 0,
                                           CHELSA_BIO_Annual_Precipitation = 0,EarthEnvTopoMed_Elevation = 0),se.fit=T) # calculate effect of human variable keeping the others constant
# ECM
pred_ECM=predict(mod2, newdata = data.frame(ConsensusLandCoverClass_Cultivated_and_Managed_Vegetation = 0,
...
# make df of residuals added to the fit for plotting the partial residuals
partial_effects=data.frame(human_orig=sample$GRIP4_DistanceToAllRoads,pred_AM=pred_AM$fit+resid(mod1),pred_ECM=pred_ECM$fit+resid(mod2),
                           pred_path=pred_path$fit+resid(mod3))
# ggplot format
